...
def train_conv_net(datasets,
    ...
    x = T.matrix('x')   
    y = T.ivector('y')
    Words = theano.shared(value = np.asarray(U), name = "Words",borrow=True) #to np. array
    ...
    Words.set_value([[]]) #per me liru memorien...     
    layer0_input=[]
    test_layer0_input=[]
    return test_perf
...
if __name__=="__main__":
    ...
    for i in r:
        stime = time.time() 
        # max_l = 56
        datasets = make_idx_data_cv(revs, word_idx_map, i, max_l=190,k=300, filter_h=5)
        perf = train_conv_net(datasets,
                              ...
                              batch_size=200,
                              dropout_rate=[0.5])
        print(("cv: " + str(i) + ", perf: " + str(perf) + " tim elapsed " + str(time.time()-stime)))
        results.append(perf)  
